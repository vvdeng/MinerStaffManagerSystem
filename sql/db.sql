...
CREATE TABLE staff(id BIGINT PRIMARY KEY AUTO_INCREMENT,NAME VARCHAR(20),card_id INT,work_id BIGINT,sex INT,birth_date VARCHAR(10),certificate_no VARCHAR(20),edu_level_id bigint,edu_level VARCHAR(10),phone VARCHAR(20), address VARCHAR(50),profession_id BIGINT, profession VARCHAR(20),department_id BIGINT,dep1_id bigint,dep2_id bigint,dep3_id bigint,department VARCHAR(20),clazz_id BIGINT,clazz VARCHAR(20),image BLOB);
...
CREATE TABLE sub_device(id BIGINT AUTO_INCREMENT PRIMARY KEY,device_id INT,`name` VARCHAR(20),state INT DEFAULT 0 ,type_id BIGINT,`type` VARCHAR(20),region_id BIGINT,region VARCHAR(50),description VARCHAR(100));
DROP TABLE IF EXISTS card_reader;
CREATE TABLE card_reader(id BIGINT AUTO_INCREMENT PRIMARY KEY,device_id INT,port_num INT,reader_id int,`name` VARCHAR(20),state INT ,type_id BIGINT,`type` VARCHAR(20),region_id BIGINT,region VARCHAR(50),description VARCHAR(100));
...
CREATE TABLE position_log(id BIGINT AUTO_INCREMENT PRIMARY KEY,card_id INT,sub_device_id INT,reader_id INT,arrive_time BIGINT,stay_time BIGINT,`event` INT ,event_desc VARCHAR(50),exist INT);
DROP TABLE IF EXISTS station_log;
CREATE TABLE station_log(id BIGINT AUTO_INCREMENT PRIMARY KEY,sub_device_id INT,arrive_time BIGINT,`event` INT ,event_desc VARCHAR(50));
#保存每个标识卡的最新位置
...
DROP TABLE IF EXISTS staff_extra;
CREATE TABLE staff_extra(staff_id BIGINT  PRIMARY KEY,card_id INT,state INT DEFAULT 0,last_arrive_time BIGINT,last_leave_time BIGINT);
