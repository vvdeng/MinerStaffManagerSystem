...
{
    CE=0;
    SPI_Write_Read_Register(WRITE_REG + CONFIG, 0x0D); 
    CE=1;
    delayUs(100);
}
...
//  SPI_Write_Read_Register(WRITE_REG + EN_RXADDR, 0x00);               // 失能接收通道0
    ...
    SPI_Write_Read_Register(WRITE_REG + RF_CH, 0/*0x01*/);                       // 选择射频通道0x01
...
//  delayUs(200);   
	...
	SPI_Write_Read_Register(FLUSH_RX,0x0);
...
void RX_Mode_2(uchar *localAddr,uchar addrLen,uchar *localPubAddr,uchar pubAddrLen)
{	
...
//   delayUs(20);
//    SPI_Write_Buffer(WRITE_REG + TX_ADDR, TX_ADDRESS, TX_ADR_WIDTH);    // 写入发送地址
    SPI_Write_Read_Register(WRITE_REG + EN_AA, 0x00);                   // 失能接收通道自动应答
//  SPI_Write_Read_Register(WRITE_REG + EN_RXADDR, 0x00);               // 失能接收通道0
    SPI_Write_Read_Register(WRITE_REG + SETUP_RETR, 0x00);              // 失能自动重发
    SPI_Write_Read_Register(WRITE_REG + RF_CH, 0/*0x01*/);                       // 选择射频通道0x01
    SPI_Write_Read_Register(WRITE_REG + RF_SETUP, 0x07);                // 数据传输率1Mbps，发射功率0dBm，低噪声放大器增益
...
//  delayUs(200);   
    ...
    SPI_Write_Buffer(WRITE_REG + RX_ADDR_P0, localAddr, addrLen);// 接收设备接收通道0使用和发送设备相同的发送地址
	SPI_Write_Read_Register(WRITE_REG + RX_PW_P0, DATA_LEN);     // 接收通道0选择和发送通道相同有效数据宽度
    SPI_Write_Buffer(WRITE_REG + RX_ADDR_P1, localPubAddr, pubAddrLen);// 接收设备接收通道1使用和发送设备相同的发送地址
	SPI_Write_Read_Register(WRITE_REG + RX_PW_P1, DATA_LEN);     // 接收通道1选择和发送通道相同有效数据宽度
   
    SPI_Write_Read_Register(WRITE_REG + EN_RXADDR, 0x03);              // 使能接收通道0和1
	SPI_Write_Read_Register(FLUSH_TX,0x0);
	SPI_Write_Read_Register(FLUSH_RX,0x0);
	SPI_Write_Read_Register(WRITE_REG + STATUS, 0xff); // 清除TX_DS或MAX_RT中断标志
    SPI_Write_Read_Register(WRITE_REG + CONFIG, 0x3f);                 // 屏蔽除去接收信号外的中断，CRC使能，16位CRC校验，上电，接收模式
