...
void Ir_work(void)
{
	...
	if(setState==SET_STATE_NONE){
		...
		displayBuf[0]=1;
		displayBuf[1]=curMenuIndex;
		displayBuf[3]=configArr[curMenuIndex]/100;
		displayBuf[4]=configArr[curMenuIndex]/10%10;
		displayBuf[5]=configArr[curMenuIndex]%10;
		...
		if(curMenuIndex==CONFIG_EXIT){
			...
			if(displayBuf[DOT_POS_INDEX]!=MENU_SEL_DOT_POS){
			 	configArr[curMenuIndex]=displayBuf[3]*100+displayBuf[4]*10+displayBuf[5];
